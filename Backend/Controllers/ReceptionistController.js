...
const createReceptionist = async (req, res) => {
  ...
  try {
    const { first_name, last_name, email, phone, password, start_date } =
      req.body;
    ...
    res
      .status(201)
      .json({ message: "Receptionist added", currentReceptionist });
  } catch (error) {
...
const getAllReceptionists = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "error finding receptionist", error });
  }
};

const getSingleReceptionists = async (req, res) => {
  try {
    const { email } = req.params;
    const foundReceptionist = await User.findOne({ email: `${email}` });
    console.log(foundReceptionist);
    res.status(200).json({ foundReceptionist });
  } catch (error) {
    console.error("error", error);
    res.status(500).json({ message: "error finding foundReceptionist", error });
  }
};
